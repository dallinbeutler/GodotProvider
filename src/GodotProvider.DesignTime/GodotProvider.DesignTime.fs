module GodotProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("GodotProvider.DesignTime", "GodotProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("GodotProvider.DesignTime", "GodotProvider.Runtime")])

    let ns = "GodotProvider"
    let asm = Assembly.GetExecutingAssembly()
